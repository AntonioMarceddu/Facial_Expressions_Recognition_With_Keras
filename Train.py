...
def get_model():
    ...
    model.compile(loss=categorical_crossentropy, optimizer=Adam(lr=learning_rate, beta_1=0.9, beta_2=0.999, epsilon=1e-6), metrics=['accuracy'])
    model.summary()
    return model
...
for train_index, validation_index in kf.split(train_images):
    ...
    lr_reducer = ReduceLROnPlateau(monitor = 'val_loss', factor = 0.95, patience = 5, mode = 'min', verbose = 1)
    ...
    score=model.evaluate(test_images, test_ground_truth, batch_size=batch_size, verbose=0)
